...
--
-- Temporary view structure for view `menunamesview`
--

DROP TABLE IF EXISTS `menunamesview`;
/*!50001 DROP VIEW IF EXISTS `menunamesview`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `menunamesview` AS SELECT 
 1 AS `MenuName`*/;
SET character_set_client = @saved_cs_client;
...
--
-- Final view structure for view `menunamesview`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`admin1`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `menunamesview` AS select `m`.`MenuName` AS `MenuName` from (`menu` `m` join `orders` `o` on((`o`.`MenuID` = `m`.`MenuID`))) where (`o`.`quantity` > 2) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`admin1`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `customerordersview` AS select `c`.`CustomerID` AS `CustomerID`,concat(`c`.`FirstName`,' ',`c`.`LastName`) AS `FullName`,`o`.`OrderID` AS `OrderID`,`o`.`TotalCost` AS `TotalCost`,`m`.`MenuName` AS `MenuName`,`mi`.`CourseName` AS `CourseName` from (((`customer_details` `c` join `orders` `o` on((`c`.`CustomerID` = `o`.`CustomerID`))) join `menu` `m` on((`o`.`MenuID` = `m`.`MenuID`))) join `menuitems` `mi` on((`mi`.`MenuItemID` = `m`.`MenuItemsID`))) order by `o`.`TotalCost` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
DELIMITER ;;
CREATE DEFINER=`admin1`@`%` PROCEDURE `AddBooking`(IN bookingid INT , IN customerid INT ,IN bookingdate date , IN tablenumber INT)
BEGIN
	INSERT INTO Bookings(BookingID, BookingDate, TableNumber, CustomerID) VALUES (bookingid, bookingdate, tablenumber, customerid);
    SELECT CONCAT('Booking ', tablenumber,' updated') AS Confirmation;
END ;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`admin1`@`%` PROCEDURE `AddValidBooking`(IN BookingDate INT , IN TableNumber INT)
BEGIN
    ...
    IF bookingExists > 0 THEN
		ROLLBACK;
        SELECT 'Table is already booked on this date. Booking declined.' AS Status;
	ELSE
...
BEGIN
    ...
    SELECT CONCAT('Order ID ', ID, ' is cancelled.') AS ConfirmationMessage;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`admin1`@`%` PROCEDURE `CheckBooking`(IN date INT , IN tablenumber INT)
BEGIN
	DECLARE bookingStatus VARCHAR(100);
    
	IF EXISTS (SELECT 1 FROM bookings where  BookingDate = date AND TableNumber = tablenumber)THEN
